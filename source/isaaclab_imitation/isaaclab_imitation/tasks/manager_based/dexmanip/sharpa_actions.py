...
def _compute_wrist_wrench_and_finger_target(
    ...
    position_error_b = quat_apply_inverse(wrist_quat_xyzw, position_error_e)
    ...
    orientation_error_b = axis_angle_from_quat(
        quat_mul(quat_inv(wrist_quat_xyzw), wrist_pose_target[:, 3:7])
    )
    torque = angular_stiffness * orientation_error_b - angular_damping * wrist_angvel_b

    # Gravity compensation (world -> body).
    gravity_force_b = quat_apply_inverse(wrist_quat_xyzw, gravity_comp_e[..., :3])
    gravity_torque_b = quat_apply_inverse(wrist_quat_xyzw, gravity_comp_e[..., 3:6])
...
@configclass
class SharpaResidualActionCfg(ActionTermCfg):
    """Source-compatible wrist-and-finger residual action configuration."""
    ...
    def __post_init__(self) -> None:
        self.class_type = JointResidualWithTrackingAction
...
class JointResidualWithTrackingAction(ActionTerm):
    ...
    cfg: SharpaResidualActionCfg
    """The configuration of the action term."""
    _scale: torch.Tensor
    """The scaling factor applied to the input action."""

    def __init__(self, cfg: SharpaResidualActionCfg, env: ManagerBasedEnv) -> None:
        ...
        if self.side == "right":
            ...
            self._wrist_pose_command_e = lambda: self.command.wrist_pose_command_xyzw(
                "right"
            )
            self._finger_joint_pos_command = lambda: (
                self.command.right_hand_finger_joint_pos_command
            )
        else:
            ...
            self._finger_joint_pos_command = lambda: (
                self.command.left_hand_finger_joint_pos_command
            )

        # Save the wrist com pose in the body frame
        wrist_com_pos_b = _tensor(self.robot.data.body_com_pos_b)[:, 0]
        self.wrist_com_pose_b = torch.zeros(
            self.num_envs, 7, dtype=wrist_com_pos_b.dtype, device=self.device
        )
        self.wrist_com_pose_b[:, :3] = wrist_com_pos_b
        self.wrist_com_pose_b[:, 6] = 1.0
        ...
        self._processed_actions[..., 6] = 1.0  # XYZW identity quaternion.
    ...
    def apply_actions(self) -> None:
        ...
        # PhysX exposes a precomputed gravity-compensation wrench on its root view.
        # Newton intentionally does not provide that view; derive the equivalent
        # world-frame force from the wrist body mass and the simulation gravity.
        root_physx_view = getattr(self.robot, "root_physx_view", None)
        if root_physx_view is not None:
            gravity_compensation = _tensor(
                root_physx_view.get_gravity_compensation_forces()
            )
        else:
            body_mass = _tensor(self.robot.data.body_mass)
            wrist_mass = body_mass[:, self.wrist_body_id]
            gravity_direction_w = _tensor(self.robot.data.GRAVITY_VEC_W)
            gravity = getattr(getattr(self._env, "sim", None), "cfg", None)
            gravity = getattr(gravity, "gravity", (0.0, 0.0, -9.81))
            gravity_magnitude = float(
                sum(component * component for component in gravity) ** 0.5
            )
            gravity_compensation = torch.zeros(
                self.num_envs, 6, device=self.device, dtype=wrist_mass.dtype
            )
            gravity_compensation[:, :3] = (
                -wrist_mass.unsqueeze(-1) * gravity_magnitude * gravity_direction_w
            )

        force, torque, finger_target = _compute_wrist_wrench_and_finger_target(
            wrist_position=self._wrist_position_e(),
            # Isaac Lab asset state and math utilities use XYZW.  The
            # command/reference boundary uses WXYZ, so keep this controller
            # input in the simulator's native order.
            wrist_quat_xyzw=_tensor(self.robot.data.root_link_quat_w),
            wrist_link_vel_w=_tensor(self.robot.data.root_com_vel_w).clone(),
            wrist_link_quat_w=_tensor(self.robot.data.root_link_quat_w),
            ...
            gravity_comp_e=gravity_compensation,
            joint_damping=_tensor(self.robot.data.joint_damping),
            joint_vel=_tensor(self.robot.data.joint_vel),
            joint_effort_limits=_tensor(self.robot.data.joint_effort_limits),
            joint_stiffness=_tensor(self.robot.data.joint_stiffness),
            joint_pos=_tensor(self.robot.data.joint_pos),
...
SharpaResidualActionCfg.class_type = JointResidualWithTrackingAction

__all__ = ["JointResidualWithTrackingAction", "SharpaResidualActionCfg"]
