...
ASSET_DIR = Path(__file__).resolve().parent / "sharpa_wave"
...
SHARPA_USD_DIR = Path(
    os.environ.get("ISAACLAB_IMITATION_SHARPA_USD_DIR", str(ASSET_DIR / "usd"))
)
"""Where pre-converted hand USD files live.

Isaac Lab converts a URDF to USD at spawn time, and that conversion needs Kit.
Shipping the converted USD lets a Newton run stay kit-less, which is how the
G1 task runs on the cluster. Generate these with
``scripts/data/convert_sharpa_assets_to_usd.py``; the task falls back to the
URDF when they are absent, so a checkout without them behaves as before.
"""


def sharpa_hand_urdf_spawn(urdf_path: str) -> Any:
    """Return the hand spawn config that imports the URDF directly.

    This is the authority on the import settings. The pre-converted USD is
    produced from it, so ``convert_sharpa_assets_to_usd.py`` asks for this
    rather than for whatever :func:`sharpa_hand_spawn` currently prefers.
    """
...
def sharpa_hand_usd_path(side: str) -> Path | None:
    """Return the pre-converted USD for one hand, if it exists."""

    for suffix in (".usda", ".usd"):
        usd = SHARPA_USD_DIR / f"{side}_sharpa_wave" / f"{side}_sharpa_wave{suffix}"
        if usd.is_file():
            return usd
    return None


def sharpa_hand_spawn(side: str, urdf_path: str) -> Any:
    """Return the hand spawn config, preferring a pre-converted USD."""

    usd = sharpa_hand_usd_path(side)
    if usd is not None:
        # Base fixity and joint drives were baked in at conversion time.
    ...
    return sharpa_hand_urdf_spawn(urdf_path)


RIGHT_SHARPA_WAVE_CFG = ArticulationCfg(
    spawn=sharpa_hand_spawn("right", right_sharpa_wave_urdf_path),
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.0, 0.0, 0.0),
        joint_pos={".*": 0.0},
        joint_vel={".*": 0.0},
...
# The primitive variants stay URDF: they are smoke-test assets and are not
# pre-converted.
...
LEFT_SHARPA_WAVE_CFG = ArticulationCfg(
    spawn=sharpa_hand_spawn("left", left_sharpa_wave_urdf_path),
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.0, 0.0, 0.0),
        joint_pos={".*": 0.0},
        joint_vel={".*": 0.0},
