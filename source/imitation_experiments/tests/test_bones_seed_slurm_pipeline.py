...
PIPELINE_SUBMITTER = REPO_ROOT / "docker/cluster/submit_job_slurm_bones_pipeline.sh"
PACE_SUBMITTER = REPO_ROOT / "docker/cluster/submit_job_slurm_pace.sh"
    ...
    env.update(
        {
            ...
            "CLUSTER_SLURM_SUBMISSION_RECORD_ROOT": str(tmp_path / "persistent_output"),
        }
    )
...
def test_qualification_audits_each_interface_cache() -> None:
    ...
    assert (
        "imitation_experiments.audit.audit_vanilla_tracker_qualification --summary " in result.stdout
...
def test_pace_submitter_emits_array_and_per_task_logs(tmp_path: Path) -> None:
    env = dict(os.environ)
    env.update(
        {
            "CLUSTER_SLURM_DRY_RUN": "1",
            "CLUSTER_SLURM_PRINT_JOB_SCRIPT": "0",
            "CLUSTER_SLURM_ARRAY": "0-2%2",
            "CLUSTER_SLURM_DEPENDENCY": "afterok:77",
    ...
    assert result.returncode == 0, result.stderr
    job_script = (tmp_path / "job.sh").read_text(encoding="utf-8")
    assert "#SBATCH --dependency=afterok:77" in job_script
    assert "#SBATCH --array=0-2%2" in job_script
    assert '#SBATCH --output="logs/slurm/%x_%A_%a.log"' in job_script
    assert '#SBATCH --error="logs/slurm/%x_%A_%a.log"' in job_script
