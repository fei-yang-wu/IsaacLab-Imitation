...
from imitation_experiments.capacity.aggregate_one_motion_capacity_seeds import CORE_METRICS, aggregate
...
def _capacity_summary(root: Path, seed: int, *, starts: list[int]) -> Path:
    ...
    mpjpe_offset = {
        "latent_skill": 0.0,
        "full_body_trajectory": 1.0,
        "ee_trajectory": 0.5,
    }
    rows = []
    for interface in ("latent_skill", "full_body_trajectory", "ee_trajectory"):
        for stage in ("demonstration_only", "rollout_finetuned"):
            ...
            mpjpe = 1.0 + seed + mpjpe_offset[interface]
            rows.append(
                {
                    "size": "tiny",
                    "interface": interface,
                    "stage": stage,
                    "parameter_count": param_by_interface[interface],
                    "target_dim": dim_by_interface[interface],
