...
from imitation_experiments.audit.audit_focused_causal_interface_comparison import (
...
def test_skynet_launcher_renders_one_seed_motion_array() -> None:
    ...
    result = subprocess.run(
        ["bash", "experiments/paper/submit_phase4_no_language_skynet.sh"],
        cwd=REPO_ROOT,
        env=env,
        check=True,
        capture_output=True,
...
def test_phase4_task_runner_dry_run_covers_each_budget(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            "bash",
            "experiments/campaigns/2026-07-23-bones-phase5-language-local10/interface_baselines/run_phase4_no_language_sweep.sh",
        ],
        cwd=REPO_ROOT,
        env=env,
        check=True,
