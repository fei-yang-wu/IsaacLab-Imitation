...
from imitation_experiments.data.planner_sample_schema import PLANNER_SAMPLE_FORMAT, PLANNER_SAMPLE_VERSION
...
def _metadata_signature(metadata: dict[str, Any]) -> str:
    ...
    if isinstance(provenance, dict):
        # Only the frozen low-level assets must match to merge samples into one
        # planner-training set. The oracle-demo collector and the planner-rollout
        # evaluator legitimately record different low_level_tracker/manifest
        # provenance shapes, so those are excluded from the merge signature.
        stable_provenance = {
