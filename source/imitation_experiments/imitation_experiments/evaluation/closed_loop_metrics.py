...
def _as_tensor(value: Any) -> torch.Tensor:
    """Bridge a Newton-backend warp ``ProxyArray`` to a torch tensor.

    IsaacLab-3 exposes ``robot.data.*`` as warp ProxyArrays under the Newton
    backend; torch/jit ops require real tensors. The ``.torch`` view is the
    repo-standard bridge and is absent on plain tensors (PhysX), so this is a
    no-op there.
    """
...
def tracking_metrics(
    ...
    *,
    ...
    # Under the Newton backend robot_data.* are warp ProxyArrays; torch/jit ops
    # (e.g. quat_error_magnitude) require real tensors. Convert via the repo's
    # standard `.torch` bridge (no-op under PhysX where these are already tensors).
