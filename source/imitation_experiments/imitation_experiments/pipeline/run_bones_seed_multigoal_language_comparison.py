...
from imitation_experiments.paths import REPO_ROOT, module_source_rel

# Campaign-owned shell launchers stay at their frozen campaign path even though
# the Python implementation now lives in the imitation_experiments package.
LAUNCHERS_REL = (
    "experiments/campaigns/2026-07-23-bones-phase5-language-local10/interface_baselines"
)
# The three-seed paper launcher is a release-facing entrypoint and lives in
# experiments/paper/, not beside the shared implementation.
PAPER_REL = "experiments/paper"
HORIZON_STEPS = 10
...
WORKFLOW_SOURCE_PATHS = (
    module_source_rel("imitation_experiments.pipeline.run_bones_seed_multigoal_language_comparison"),
    module_source_rel("imitation_experiments.audit.validate_bones_seed_planner_submission"),
    module_source_rel("imitation_experiments.data.collect_interface_rollout_samples"),
    ...
    module_source_rel("imitation_experiments.planner.train_chunked_transformer_planner"),
    module_source_rel("imitation_experiments.evaluation.eval_interface_planner_offline"),
    module_source_rel("imitation_experiments.data.merge_planner_samples"),
    module_source_rel("imitation_experiments.data.planner_sample_schema"),
    module_source_rel("imitation_experiments.planner.interface_planner_common"),
    module_source_rel("imitation_experiments.audit.audit_bones_seed_multigoal_language_comparison"),
    module_source_rel("imitation_experiments.evaluation.summarize_bones_seed_multigoal_language_comparison"),
    f"{LAUNCHERS_REL}/submit_bones_seed_multigoal_pipeline_skynet.sh",
    f"{PAPER_REL}/submit_bones_seed_multiseed_pipeline_skynet.sh",
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--rollout_command_source",
        choices=("planner", "oracle"),
        default="planner",
        help=(
            "Data source for the rollout-collection stage. 'planner' (default, "
            "the paper protocol) is DAgger: the pretrained planner drives the "
            "robot. 'oracle' drives with the frozen skill encoder, i.e. it "
            "collects additional expert-distribution data instead of on-policy "
            "data. Only affects the latent_skill rollout collection."
        ),
    )
    parser.add_argument(
        "--extra_finetune_samples",
        type=Path,
        default=None,
        help=(
            "Optional extra sample pool merged into the finetune dataset in "
            "addition to demonstration + rollout samples (e.g. an oracle-rollout "
            "pool to form a demo+planner+oracle composition). Ablation-only; "
            "unset preserves the paper protocol exactly."
        ),
    )
    parser.add_argument(
        "--rollout_num_envs",
...
def _latent_eval_command(
    ...
    *,
    ...
    command.append("physics=newton_mjwarp")
    # BONES-SEED motions exceed the LAFAN1-tuned Newton contact budget
    # (njmax=95/nconmax=10) and overflow corrupts the dynamics; 320/40 is the
    # validated zero-overflow headroom the current controllers trained with.
    command.extend(
        [
...
def main() -> None:
    ...
    if args.stage in {"all", "rollout"}:
        ...
        for goal_index in rollout_goal_indices:
            ...
            if "latent_skill" in selected:
                ...
                rollout_is_oracle = args.rollout_command_source == "oracle"
                runner.run(
                    _latent_eval_command(
                        runner,
                        goal=goal,
                        ...
                        command_source=(
                            "hl_skill" if rollout_is_oracle else "skill_commander"
                        ),
                        planner_checkpoint=(
                            None
                            if rollout_is_oracle
                            else pretrain_checkpoints["latent_skill"]
                        ),
                        save_samples=True,
                        refresh=False,
    ...
    for interface in active_interfaces:
        ...
        if args.stage in {"all", "finetune"}:
            ...
            final_sources = [merged_demo[interface], merged_rollout]
            final_limits = [0, 0]
            if args.extra_finetune_samples is not None:
                final_sources.append(_resolve(args.extra_finetune_samples))
                final_limits.append(0)
            runner.run(
                _merge_command(
                    runner,
    ...
    runner.run(
        [
            *runner.python,
            "-m",
        "imitation_experiments.evaluation.summarize_bones_seed_multigoal_language_comparison",
            "--run_root",
            str(output_root),
            "--output_json",
            str(summary_json),
    ...
    if run_paper_audit:
        runner.run(
            [
                *runner.python,
                "-m",
        "imitation_experiments.audit.audit_bones_seed_multigoal_language_comparison",
                "--run_root",
                str(output_root),
                "--output_json",
                str(audit_json),
