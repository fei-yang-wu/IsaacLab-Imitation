...
| --- | --- | --- |
| Active (SUBMITTED 2026-08-23) | [`2026-08-22-reward-estimation`](campaigns/2026-08-22-reward-estimation/README.md) | IPMD direct reward estimation (IRL) on the tuned explicit root_qpos tracker, 10B, one arm. The reward input is fixed and normalized into [0, 1] on the v2 surface (`RewardInputUnitCfg`: soft-limit joint positions + relative anchor pos/rot6d), expert side served through the same helpers. PPO still trains on the task reward, so the row doubles as an explicit baseline. ICE chain 5588194 -> 5588195 -> 5588196; W&B project `g1-reward-estimation`, group `irl-explicit-10b`. |
