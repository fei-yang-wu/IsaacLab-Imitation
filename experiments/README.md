...
| --- | --- | --- |
| PARKED (complete 2026-09-06) | [`2026-09-04-direct-affine-phi`](campaigns/2026-09-04-direct-affine-phi/README.md) | One 10B arm, 12,288 environments after two physics out-of-memory deaths at 20,480 and 16,384. 64-D phi plus two phase values, weight decay, linear critic schedule, LayerNorm past-5 affine encoder, and a 256-unit LSTM actor; no explicit actor history. Finished at exactly 10,000,171,008 frames. **One row, one seed:** 0.8474 SR / 22.92 mm MPJPE-L / 65.97 mm MPJPE-G -- the lowest MPJPE-G and the lowest SR of the comparison set, with three variables against `combo`, so it attributes nothing alone. The recurrent-actor axis is parked with `2026-09-02-lstm-hub64-10b`; reviving it needs a pair that moves the actor alone against a history-free MLP, one batch shape, repeated seeds, and the recurrent-state reset zeroing in `evaluate_checkpoint` verified first. |
