#!/usr/bin/env bash
# EC grading of the expert-heading encoder arms on the deployment host, the
# same pass as 2026-09-13-e5-hardware-gap-10b/ec_grade_echost.sh: download
# the newest tracker checkpoint from ICE, export a `combo_v3` hold-1 bundle
# with the ARM'S OWN encoder (downloaded once from ICE), push it to echost,
# rehearse asynchronously there (DDS plant, measured sensor noise, cuda
# inference, `--anchor expert_heading`, one video per motion) and grade it
# from the plant's true state. One row per graded checkpoint lands in
# `logs/expert_heading_encoders_ec/results_echost.tsv` (same columns as the
# e5 pass, so the two tables concatenate).
#
# Only `eh` can be graded here: the EC native oracle builds the 38-per-frame
# window and has no end-effector terms yet, so an `eh_ee` bundle (500-wide
# encoder input) cannot be rehearsed until `native_fake_runtime` grows the
# `expert_ee_pos_b` points. The script refuses `eh_ee` with a message.
#
# Takes the e5 pass's lock (`logs/e5_hardware_gap_ec/.lock`): echost runs the
# four plant lanes and nothing else may share them.
#
#   ./ec_grade_echost.sh                 # eh, newest checkpoint
#   FORCE_FRAMES=73000000000 ./ec_grade_echost.sh
set -uo pipefail
...
ARMS="${ARMS:-eh}"
LANES="${LANES:-4}"
ROOT="${REPO_ROOT}/logs/expert_heading_encoders_ec"
LOCK="${REPO_ROOT}/logs/e5_hardware_gap_ec/.lock"
mkdir -p "${ROOT}/ckpt" "${ROOT}/bundles" "${ROOT}/grades" "${ROOT}/encoders" "$(dirname "${LOCK}")"
...
for arm in ${ARMS}; do
    if [ "${arm}" = "eh_ee" ]; then
        log "[ehenc_eh_ee] skipped: the EC native oracle has no end-effector window terms (38 per frame only)"
        continue
    fi
    ...
    [ -n "${newest}" ] || { log "[ehenc_${arm}] no checkpoint yet"; continue; }
    name="ehenc_${arm}_f${newest}"
    if grep -q "^${arm}	${newest}	" "${RESULTS}"; then log "[${name}] already graded"; continue; fi

    encoder="${ROOT}/encoders/${arm}_latest.pt"
    if [ ! -s "${encoder}" ]; then
        remote_enc="${DATA}/expert_heading_encoders_10b/${arm}_seed0/encoder/checkpoints/latest.pt"
        timeout 90 ssh "${REMOTE}" "test -s ${remote_enc}" || { log "[${name}] encoder not on ${REMOTE} yet"; continue; }
        log "[${name}] download encoder ${remote_enc}"
        scp -q "${REMOTE}:${remote_enc}" "${encoder}.part" && mv "${encoder}.part" "${encoder}" || { log "[${name}] encoder download failed"; continue; }
    fi
    ...
    if [ ! -f "${bundle}/manifest.json" ]; then
        log "[${name}] export combo_v3 hold 1 with the ${arm} encoder"
        rm -rf "${bundle}"
    ...
    anchor_flag="--anchor expert_heading"
    out="artifacts/ehenc_echost/${name}"
    log "[${name}] echost: verify + rehearse all --lanes ${LANES} --plant-noise measured ${anchor_flag}"
    timeout 3600 ssh "${ECHOST}" "export PATH=\$HOME/.pixi/bin:\$PATH; cd ${ECHOST_EC} && mkdir -p artifacts/ehenc_echost && rm -rf ${out} && \
