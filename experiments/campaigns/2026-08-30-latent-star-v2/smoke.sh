#!/usr/bin/env bash
# Wiring qualification for the latent-star-v2 campaign. Proves an arm RUNS; it
...
SMOKE_ROOT="${SMOKE_ROOT:-${REPO_ROOT}/logs/latent_star_v2_smoke}"
...
tokens += ["--horizon_steps", str(merged["horizon"])]
...
# Newline-separated token list for a per-arm list var, ${vars.X} indirection
# resolved (the dyn arms set finetune_args: ${vars.dyn_block}).
arm_token_list() {
...
for arm in ${ARMS}; do
    ...
    # Group 1's posterior arms take a different entry point and command
    # source, and they have no pretrain stage to smoke. Their wiring is
    # qualified by the posterior campaign's own runner, not this one.
    if [[ "$(arm_field "${arm}" route)" == "posterior" ]]; then
        log "[SKIP] ${arm} is a posterior-route arm; smoke it with the posterior runner"
        continue
    fi
    mkdir -p "${work}"
    ...
    horizon="$(arm_field "${arm}" horizon)"
    hold="$(arm_field "${arm}" hold)"
    phase_mode="$(arm_field "${arm}" phase_mode)"
    phase_source="$(arm_field "${arm}" phase_source)"
    phase_period="$(arm_field "${arm}" phase_period)"
    ...
    mapfile -t finetune_tokens < <(arm_token_list "${arm}" finetune_args)
    mapfile -t reward_tokens < <(arm_token_list "${arm}" reward_args)
    mapfile -t extra_tokens < <(arm_token_list "${arm}" extra_args)
