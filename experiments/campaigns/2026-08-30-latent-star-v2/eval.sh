#!/usr/bin/env bash
# Score latent-star-v2 checkpoints.
#
...
#   ARMS="hub g3_fsq64" ROWS=clean ./eval.sh
#   ./eval.sh --report
set -uo pipefail
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/logs/latent_star_v2_eval}"
...
campaign = yaml.safe_load(
    pathlib.Path(
        "experiments/campaigns/2026-08-30-latent-star-v2/campaign.yaml"
    ).read_text()
)
...
mirrors = {
    "/data/latent_star_v2/": "logs/latent_star_v2_mirror/",
...
fields = {
    ...
    # `horizon` moves with g4_h5 / g4_h20 and `route` with Group 1's posterior
    # arms. Both were hardcoded in the campaign this runner was copied from.
    "horizon": int(merged["horizon"]),
    "route": merged.get("route", "pretrained"),
    "quantizer": merged.get("quantizer", "identity"),
    "through_policy": merged.get("through_policy", "true"),
    "recon_coeff": merged.get("recon_coeff", "1.0"),
    "kl_coeff": merged.get("kl_coeff", "0.0"),
    "posterior_space": " ".join(merged.get("posterior_space_args") or []),
}
...
import yaml
campaign = yaml.safe_load(open('experiments/campaigns/2026-08-30-latent-star-v2/campaign.yaml'))
print('\n'.join(campaign['arms']))
")
    ARMS="${all_arms[*]}"
fi
...
for arm in ${ARMS}; do
  for seed in ${SEEDS:-$(arm_seed "${arm}")}; do
    ...
    # Group 1's posterior arms have no encoder file by construction: the code
    # is learned during RL and lives inside the tracker checkpoint.
    if [[ "${route}" != "posterior" && ! -s "${encoder}" ]]; then
        log "[SKIP] no encoder ${encoder}"; continue
    fi
...
")
        ...
        # The route decides the agent entry point and how the command is
        # produced. Group 1's posterior arms have no encoder file at all: the
        # code lives inside the tracker checkpoint.
        if [[ "${route}" == "posterior" ]]; then
            entry_point=rlopt_ipmd_posterior_root_qpos_cfg_entry_point
            route_args=(
                ...
                "agent.ipmd.latent_learning.quantizer=${quantizer}"
                agent.ipmd.latent_learning.patch_past_steps=0
                agent.ipmd.latent_learning.patch_future_steps=9
                "agent.ipmd.latent_learning.train_posterior_through_policy=${through_policy}"
                "agent.ipmd.latent_learning.recon_coeff=${recon_coeff}"
                "agent.ipmd.latent_learning.kl_coeff=${kl_coeff}"
                ${posterior_space:+${posterior_space}}
            )
        else
            entry_point=rlopt_ipmd_tuned_cfg_entry_point
            route_args=(
