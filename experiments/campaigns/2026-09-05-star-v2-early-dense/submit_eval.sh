#!/usr/bin/env bash
# Plan one arm of the star-v2 early-dense curve-eval campaign through the cluster control plane.
...
# command. The W&B project is g1-bs-ablation and the group is latent-star-v2-early.
set -euo pipefail
...
exec pixi run python -m imitation_experiments.pipeline.cluster plan \
    --campaign "${CAMPAIGN_DIR}/eval_campaign.yaml" --arm "${ARM}" --seed "${SEED}" "$@"
