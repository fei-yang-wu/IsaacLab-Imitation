#!/usr/bin/env bash
# Submit every arm's early-dense curve evaluation, then retry the ones that die in Kit
# startup.
#
...
import yaml
print('\n'.join(yaml.safe_load(open('${CAMPAIGN_DIR}/eval_campaign.yaml'))['arms']))
")
fi
...
mapfile -t busy < <(ssh "${REMOTE_HOST}" \
    'squeue -u $USER -h -o "%j" | grep "^star-v2-early-curves" | sed -E "s/star-v2-early-curves-(.*)-s0-score/\1/"' 2>/dev/null)

skip=" ${busy[*]} "
for arm in "${all[@]}"; do
    [[ "${skip}" == *" ${arm} "* ]] && { printf '[skip] %s\n' "${arm}"; continue; }
    out="$("${CAMPAIGN_DIR}/submit_eval.sh" "${arm}" 0 2>&1)"
