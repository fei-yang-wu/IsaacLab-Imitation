#!/usr/bin/env bash
# Score the newest tracker checkpoint of each ankle-limit arm
# (c1l1, c1l2) on the standard board, skipping checkpoints already scored or
# in flight.
#
# Trees: /data/ankle_limit_c1_10b/<arm>_seed0/tracker. The newest
# model_step_*.pt is relinked into a milestone-layout tree under
# /data/ankle_limit_c1_10b_live/ankl_<arm>_seed0/tracker/f<frames>/
# and scored by the latest-eval arm `ankl_<arm>_live`, whose row lands as
# latest_eval/ankl_<arm>_seed0_clean_f<frames>.json.
#
...
prepare_worktree() {
    [ -n "${WORKTREE}" ] && return 0
    WORKTREE="${REPO_ROOT}/.claude/worktrees/live-eval-ankl-$$"
...
for arm in ${ARMS}; do
    ...
    live_root="${DATA}/ankle_limit_c1_10b_live/ankl_${arm}_seed0/tracker"
    newest="$(timeout 90 ssh "${REMOTE}" "ls ${tree}/*/models/model_step_*.pt 2>/dev/null | sed 's|.*/||' | sed -E 's/model_step_([0-9]+)\.pt/\1/' | sort -n | tail -1")"
    [ -n "${newest}" ] || { echo "[ankl_${arm}] no checkpoint yet"; continue; }
    if timeout 90 ssh "${REMOTE}" "test -s ${EVAL_DIR}/ankl_${arm}_seed0_clean_f${newest}.json"; then
        echo "[ankl_${arm}] f${newest} already scored; nothing to do"; continue
    fi
    if timeout 90 ssh "${REMOTE}" "squeue -u fwu91 -h -o '%j' | grep -q 'latest-eval-ankl_${arm}_live'"; then
        echo "[ankl_${arm}] an eval is already in flight; skipping"; continue
    fi
    echo "=== ankl_${arm}: newest f${newest} unscored"
    if [ "${DRY_RUN}" = "1" ]; then echo "[ankl_${arm}] would relink and submit"; continue; fi
    ...
    out="$(cd "${WORKTREE}" && PYTHONPATH="${WORKTREE}/source/imitation_experiments" timeout 900 "${PY}" -m imitation_experiments.pipeline.cluster plan \
        --campaign "${EVAL_CAMPAIGN}" --arm "ankl_${arm}_live" --seed 0 \
    ...
    [ -n "${plan}" ] || { echo "[ankl_${arm}] plan failed"; echo "${out}" | tail -20; continue; }
    ...
    echo "[ankl_${arm}] row will land at ${EVAL_DIR}/ankl_${arm}_seed0_clean_f${newest}.json"
done
