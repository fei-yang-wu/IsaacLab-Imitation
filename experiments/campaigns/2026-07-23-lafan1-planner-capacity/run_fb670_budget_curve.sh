...
stage_collect() {
    ...
    run_if_missing "${SAMPLES_DIR}/sample_step_000000.pt" \
        "${ISAAC_PY_ARR[@]}" "${SHARED_DIR}/collect_interface_rollout_samples.py" \
...
eval_one() {
    ...
    run_if_missing "${out}/summary.json" \
        "${ISAAC_PY_ARR[@]}" "${SHARED_DIR}/eval_interface_planner_closed_loop.py" \
        --headless --device "${DEVICE}" \
        --task "${CHUNK_TASK}" --algorithm IPMD \
        --checkpoint "${FB_LOW_LEVEL_CHECKPOINT}" \
...
stage_aggregate() {
    ...
    "${PLAIN_PY_ARR[@]}" \
        experiments/campaigns/2026-07-23-lafan1-planner-capacity/interface_baselines/aggregate_planner_budget_curve.py \
