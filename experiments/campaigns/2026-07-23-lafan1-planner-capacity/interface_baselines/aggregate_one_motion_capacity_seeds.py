...
def aggregate(
    *,
    ...
    # Aggregate over the interfaces actually present in every per-seed payload
    # (e.g. latent + full_body when the EE row is not part of the run).
    active_interfaces = tuple(
        interface
        for interface in INTERFACES
        if all(interface in payload.get("oracles", {}) for payload in payloads)
    )
    if "latent_skill" not in active_interfaces or len(active_interfaces) < 2:
        raise ValueError(
            "Need latent_skill plus at least one explicit interface in every "
            f"per-seed payload; got {active_interfaces}"
        )
    oracle_hashes = {
    ...
    for (seed, family, size, stage), pair in sorted(by_point.items()):
        if "latent_skill" not in pair:
            continue
        latent = pair["latent_skill"]
        diffs: dict[str, dict[str, float]] = {}
        for explicit_interface in EXPLICIT_INTERFACES:
            explicit = pair.get(explicit_interface)
            if explicit is None:
                continue
            diffs[explicit_interface] = {
        ...
        paired_rows.append(
            {
                ...
                "size": size,
                "stage": stage,
                # latent_minus_explicit keyed by which explicit interface is paired.
                "latent_minus_explicit": diffs,
            }
        )
