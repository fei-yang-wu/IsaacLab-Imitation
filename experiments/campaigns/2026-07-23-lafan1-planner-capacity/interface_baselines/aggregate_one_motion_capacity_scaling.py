...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Frozen low-level oracle summary per interface. Repeatable. "
        "Interfaces: " + ", ".join(INTERFACES) + ".",
    )
    # Backward-compatible convenience aliases (map to --oracle entries).
    parser.add_argument("--latent_oracle", type=Path, default=None)
    parser.add_argument("--explicit_oracle", type=Path, default=None)
    parser.add_argument("--ee_oracle", type=Path, default=None)
    parser.add_argument(
...
def aggregate(
    *,
    ...
    if "latent_skill" not in active_interfaces or len(active_interfaces) < 2:
        raise ValueError(
            "Need latent_skill plus at least one explicit interface oracle; "
            f"got {list(oracle_paths)}"
        )
    ...
    oracles = {
        iface: _oracle_record(oracle_paths[iface]) for iface in active_interfaces
    }
...
def main() -> None:
    ...
    for item in args.oracle:
        if "=" not in item:
            raise ValueError(f"Expected --oracle INTERFACE=PATH, got {item!r}")
        interface, raw_path = item.split("=", 1)
        if interface not in INTERFACES:
            raise ValueError(f"Unknown --oracle interface {interface!r}")
        if interface in oracle_paths:
            raise ValueError(f"Duplicate --oracle for {interface!r}")
        oracle_paths[interface] = Path(raw_path)
    for interface, alias in (
        ("latent_skill", args.latent_oracle),
        ("full_body_trajectory", args.explicit_oracle),
        ("ee_trajectory", args.ee_oracle),
    ):
        if alias is not None:
            if interface in oracle_paths:
                raise ValueError(f"Oracle for {interface!r} given twice")
            oracle_paths[interface] = alias
