#!/usr/bin/env bash
# Regular Embodied-Control grading of the e5 hardware-gap arms (cloned from the energy campaign
...
# checkpoint lands in `logs/e5_hardware_gap_ec/results.tsv`.
#
...
for arm in ${ARMS}; do
    ...
    log "[${name}] rehearse all --lanes ${LANES} --plant-noise measured ${anchor_flag} (videos per motion)"
    rm -rf "${out}"
    pixi run -e native ec lifecycle rehearse "assets/models/controller/${name}" all --output "${out}" --lanes "${LANES}" --dds-domain-base 110 --plant-noise measured ${anchor_flag} > "${out}.log" 2>&1
