...
# Walk up to the repo root via the pixi.toml marker. Do NOT use git here: on the
# cluster this runs inside an extracted workspace tarball with no .git, where
# `git rev-parse` exits 128 and `set -e` would kill the job during setup.
REPO_ROOT="${SCRIPT_DIR}"
while [[ "${REPO_ROOT}" != "/" && ! -f "${REPO_ROOT}/pixi.toml" ]]; do
...
ROLLOUT_COMMAND_SOURCE="${ROLLOUT_COMMAND_SOURCE:-planner}"
EXTRA_FINETUNE_SAMPLES="${EXTRA_FINETUNE_SAMPLES:-}"
...
cmd=(
    ...
    --rollout_command_source "${ROLLOUT_COMMAND_SOURCE}"
...
if [[ -n "${EXTRA_FINETUNE_SAMPLES}" ]]; then
    cmd+=(--extra_finetune_samples "${EXTRA_FINETUNE_SAMPLES}")
fi
