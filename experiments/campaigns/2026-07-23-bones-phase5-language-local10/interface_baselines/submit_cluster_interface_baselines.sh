...
# Resolve the repo root robustly; the 2026-07-23 reorg added a `paper/` level
# under experiments/, which broke the old fixed `../..` relative path.
REPO_ROOT="$(cd "${SCRIPT_DIR}" && { git rev-parse --show-toplevel 2>/dev/null || (cd ../../../.. && pwd); })"
cd "${REPO_ROOT}"

MODE="${MODE:?Set MODE to an active workflow; see experiments/README.md}"
...
ENV_KEYS=(
    ...
    LATENT_ALGORITHM
    LATENT_LOW_LEVEL_CHECKPOINT
    LATENT_LOW_LEVEL_RUN_ID
    EXPECTED_LATENT_CHECKPOINT_BASENAME
    LATENT_SKILL_CHECKPOINT
    LATENT_PLANNER_CHECKPOINT
    LATENT_LANGUAGE_EMBEDDINGS
    LATENT_MOTION_NAME
