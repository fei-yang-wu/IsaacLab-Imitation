...
# ICE submission for ablation ARM A (DAgger + demo) — the standard multi-goal
# pipeline (demo oracle collection + planner-driven rollout) at the LARGE
# planner and 1000 rows/motion. This also produces the shared pools (demo,
# pretrained-large planner, planner-rollout) that arms B/C reuse.
#
# Runs against the ICE-staged full 100-motion Phase-5 tree, restricted to the
# ten ablation goals via GOAL_NAMES. Latent-only, preliminary/unqualified.
#
# Dry-run (default):
#   experiments/campaigns/2026-07-23-bones-phase5-language-local10/submit_ablation_arm_a_ice.sh
# Real submit:
#   DRY_RUN=0 CONFIRM_SUBMIT=I_UNDERSTAND_UNQUALIFIED_PHASE5 \
#     experiments/campaigns/2026-07-23-bones-phase5-language-local10/submit_ablation_arm_a_ice.sh
...
OUTPUT_ROOT="${OUTPUT_ROOT:-logs/interface_baselines/bones_seed_phase5_ice_ablation_A_dagger_demo_seed0}"
...
export CLUSTER_SLURM_JOB_NAME_PREFIX="${CLUSTER_SLURM_JOB_NAME_PREFIX:-bones-abl-A}"
