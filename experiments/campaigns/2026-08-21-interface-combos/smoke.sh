#!/usr/bin/env bash
# Wiring qualification for the interface combos campaign. Proves an arm RUNS; it
...
SMOKE_ROOT="${SMOKE_ROOT:-${REPO_ROOT}/logs/interface_combos_smoke}"
...
campaign = yaml.safe_load(
    pathlib.Path(
        "experiments/campaigns/2026-08-21-interface-combos/campaign.yaml"
    ).read_text()
)
...
import yaml
campaign = yaml.safe_load(open('experiments/campaigns/2026-08-21-interface-combos/campaign.yaml'))
