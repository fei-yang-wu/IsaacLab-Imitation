#!/usr/bin/env bash
# Score the newest tracker checkpoint of each ankle-termination arm
# (c1l1t1, c1l1j1) on the standard board, skipping checkpoints already scored or
# in flight.
#
# Trees: /data/ankle_term_c1l1_10b/<arm>_seed0/tracker. The newest
# model_step_*.pt is relinked into a milestone-layout tree under
# /data/ankle_term_c1l1_10b_live/ankt_<arm>_seed0/tracker/f<frames>/
# and scored by the latest-eval arm `ankt_<arm>_live`, whose row lands as
# latest_eval/ankt_<arm>_seed0_clean_f<frames>.json.
#
# The plan and submit run from a clean detached worktree of HEAD (with the
# submodules at their recorded commits) instead of `git stash`, so another
# session's uncommitted edits never enter the archive.
#
#   ./submit_live_eval.sh              # both arms
#   ARMS="c1l1j1" ./submit_live_eval.sh
...
prepare_worktree() {
    [ -n "${WORKTREE}" ] && return 0
    WORKTREE="${REPO_ROOT}/.claude/worktrees/live-eval-ankt-$$"
...
for arm in ${ARMS}; do
    tree="${DATA}/ankle_term_c1l1_10b/${arm}_seed0/tracker"
    live_root="${DATA}/ankle_term_c1l1_10b_live/ankt_${arm}_seed0/tracker"
    newest="$(timeout 90 ssh "${REMOTE}" "ls ${tree}/*/models/model_step_*.pt 2>/dev/null | sed 's|.*/||' | sed -E 's/model_step_([0-9]+)\.pt/\1/' | sort -n | tail -1")"
    [ -n "${newest}" ] || { echo "[ankt_${arm}] no checkpoint yet"; continue; }
    if timeout 90 ssh "${REMOTE}" "test -s ${EVAL_DIR}/ankt_${arm}_seed0_clean_f${newest}.json"; then
        echo "[ankt_${arm}] f${newest} already scored; nothing to do"; continue
    fi
    if timeout 90 ssh "${REMOTE}" "squeue -u fwu91 -h -o '%j' | grep -q 'latest-eval-ankt_${arm}_live'"; then
        echo "[ankt_${arm}] an eval is already in flight; skipping"; continue
    fi
    echo "=== ankt_${arm}: newest f${newest} unscored"
    if [ "${DRY_RUN}" = "1" ]; then echo "[ankt_${arm}] would relink and submit"; continue; fi
    ...
    out="$(cd "${WORKTREE}" && PYTHONPATH="${WORKTREE}/source/imitation_experiments" timeout 900 "${PY}" -m imitation_experiments.pipeline.cluster plan \
        --campaign "${EVAL_CAMPAIGN}" --arm "ankt_${arm}_live" --seed 0 \
    ...
    [ -n "${plan}" ] || { echo "[ankt_${arm}] plan failed"; echo "${out}" | tail -20; continue; }
    ...
    echo "[ankt_${arm}] row will land at ${EVAL_DIR}/ankt_${arm}_seed0_clean_f${newest}.json"
done
