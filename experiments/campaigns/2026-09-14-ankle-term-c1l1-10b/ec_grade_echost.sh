#!/usr/bin/env bash
# EC grading of the ankle-termination arms (c1l1t1, c1l1j1) on the deployment host, the
...
#   ARMS="c1l1j1" ./ec_grade_echost.sh
set -uo pipefail
...
ROOT="${REPO_ROOT}/logs/ankle_term_ec"
...
for arm in ${ARMS}; do
    ...
    [ -n "${newest}" ] || { log "[ankt_${arm}] no checkpoint yet"; continue; }
    name="ankt_${arm}_f${newest}"
    ...
    anchor_flag=""
    out="artifacts/ankt_echost/${name}"
    log "[${name}] echost: verify + rehearse all --lanes ${LANES} --plant-noise measured ${anchor_flag}"
    timeout 3600 ssh "${ECHOST}" "export PATH=\$HOME/.pixi/bin:\$PATH; cd ${ECHOST_EC} && mkdir -p artifacts/ankt_echost && rm -rf ${out} && \
