#!/usr/bin/env bash
# Oracle-policy ROLLOUT collection over the 30 compositionality motions.
#
# "Rollout data" = the frozen tracker actually drives the robot from the
# oracle latent command, so the causal state history carries real closed-loop
# dynamics AND this tracker's own `last_action`. Rows land at every
...
#
# The z256 variant of this collection already exists as
# `logs/bones_language30_compositionality_oracle_seed0/collection`; only the
# fsq64 variant needs to be produced here.
#
# Usage: collect_rollout.sh <z256|fsq64>
#
# Unlike the replay pair, two rollout collections do NOT share state
# histories: each tracker drives its own trajectory, so both the dynamics and
# the `last_action` block differ. That is the point — a rollout arm sees the
# `last_action` distribution of the tracker it will be deployed against.
set -euo pipefail
...
OUTPUT_DIR="${REPO_ROOT}/logs/gr00t_language30_rollout_${VARIANT}/collection"
...
pixi run -e isaaclab python scripts/rlopt/eval_skill_commander_closed_loop.py \
    ...
    --label "gr00t_language30_rollout_${VARIANT}" \
    --num_envs 3000 --max_steps 1200 --seed 0 --metric_interval 1200 \
