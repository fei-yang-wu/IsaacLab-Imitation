...
cmd=(
    pixi run -e isaaclab python experiments/multicat_qualitative_analysis.py multi
    ...
    --stride "${STRIDE:-10}" --motion-count "${MOTION_COUNT:-50}"
    --max-chunks-per-motion "${MAX_CHUNKS_PER_MOTION:-20}" --seed "${SEED:-0}"
