...
parser.add_argument(
    ...
    default=False,
    help=(
        "Before env creation, load only the manifest motion selected by "
        "--policy_trajectory_rank. The selected trajectory becomes rank 0."
    ),
)
parser.add_argument(
    "--policy_motion",
...
def _resolve_manifest_cli_overrides(env_cfg) -> None:
    """Resolve a CLI manifest without discarding an explicit CLI dataset path."""
    loader_kwargs = getattr(env_cfg, "loader_kwargs", {})
    entries = (
        loader_kwargs.get("dataset", {}).get("trajectories", {}).get("lafan1_csv", [])
    )
    if getattr(env_cfg, "lafan1_manifest_path", None) is None or entries:
        return

    override_names = {
        item.split("=", 1)[0].lstrip("+") for item in hydra_args if "=" in item
    }
    resolver = getattr(env_cfg, "_resolve_manifest_config", None)
    if callable(resolver):
        resolver(
            dataset_path_explicit="env.dataset_path" in override_names,
            motions_explicit="env.motions" in override_names,
            timing_explicit=bool(
                {"env.sim", "env.sim.dt", "env.decimation"} & override_names
            ),
        )


def _restrict_dataset_to_policy_trajectory(env_cfg) -> None:
    if not args_cli.restrict_dataset_to_policy_trajectory:
        return
    if args_cli.policy_trajectory_rank is None:
        raise ValueError(
            "--restrict_dataset_to_policy_trajectory requires --policy_trajectory_rank."
        )

    manifest_path = getattr(env_cfg, "lafan1_manifest_path", None)
    if manifest_path is None:
        raise ValueError(
            "--restrict_dataset_to_policy_trajectory requires env.lafan1_manifest_path."
        )
    _, entries = load_lafan1_manifest(manifest_path)
    original_rank = int(args_cli.policy_trajectory_rank)
    if not 0 <= original_rank < len(entries):
        raise ValueError(
            f"--policy_trajectory_rank must be in [0, {len(entries) - 1}], "
            f"got {original_rank}."
        )

    motion_name = str(entries[original_rank]["name"])
    env_cfg.motions = [motion_name]
    args_cli.policy_trajectory_rank = 0
    print(
        "[INFO] Restricted reference cache loading to manifest rank "
        f"{original_rank}: {motion_name!r}."
    )
...
def _enable_newton_video_visualizer(env_cfg) -> None:
    if type(env_cfg.sim.physics).__name__ != "NewtonCfg":
        return
    ...
    visualizer = NewtonVisualizerCfg(
        ...
        eye=(3.0, -5.0, 2.9),
        lookat=(0.0, 1.25, 0.9),
        visible_env_indices=[REFERENCE_ENV_ID, POLICY_ENV_ID],
    )
    existing = list(getattr(env_cfg.sim, "visualizer_cfgs", None) or [])
    env_cfg.sim.visualizer_cfgs = existing + [visualizer]
    print("[INFO] Enabled kit-less NewtonGL side-by-side video rendering.")
...
def _run(
    ...
    _resolve_manifest_cli_overrides(env_cfg)
    _restrict_dataset_to_policy_trajectory(env_cfg)
...
def main(
    ...
    if args_cli.video:
        _enable_newton_video_visualizer(env_cfg)
