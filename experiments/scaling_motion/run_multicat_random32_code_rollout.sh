...
cmd=(
    ...
    --headless --device cuda:0 --video --independent-random-codes
    ...
    --output-root "${OUTPUT_ROOT:-outputs/qualitative_analysis/bs5000-multicat-random${category_count}-codes}"
    --motion "${MOTION:-jog_ff_loop_180_R_002_A091_M}"
    --rollout-steps "${ROLLOUT_STEPS:-100}" --seed "${SEED:-0}"
