...
# Strict corrected-LAFAN1 low-level gate. This evaluates the two frozen
# controllers and certifies the streamed vanilla adapter. It never trains or
# submits a high-level planner.
...
: "${MANIFEST:?Set MANIFEST to the corrected LAFAN1 manifest}"
: "${DATASET_PATH:?Set DATASET_PATH to the corrected LAFAN1 cache}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-logs/interface_baselines/lafan1_corrected_low_level_qualification_seed0}"
...
if [[ "${NUM_ENVS}" != "40" || "${EVAL_STEPS}" != "1000" || "${SEED}" != "0" ]]; then
    echo "[ERROR] Final LAFAN1 qualification requires NUM_ENVS=40, EVAL_STEPS=1000, and SEED=0." >&2
    exit 2
fi
...
if [[ "${MIN_ORACLE_SUCCESS}" != "0.8" ]]; then
    echo "[ERROR] Final LAFAN1 qualification requires MIN_ORACLE_SUCCESS=0.8." >&2
    exit 2
fi
...
if [[ -z "${LATENT_LOW_LEVEL_CHECKPOINT}" ]]; then
    if [[ -z "${LATENT_LOW_LEVEL_RUN_ID}" ]]; then
        echo "[ERROR] Set LATENT_LOW_LEVEL_CHECKPOINT or LATENT_LOW_LEVEL_RUN_ID." >&2
        exit 2
    fi
    if [[ "${DRY_RUN}" == "1" || "${DRY_RUN}" == "true" ]]; then
        LATENT_LOW_LEVEL_CHECKPOINT="logs/rlopt/ipmd/Isaac-Imitation-G1-Latent-v0/DRY_RUN/models/${EXPECTED_LATENT_CHECKPOINT_BASENAME}"
    else
        latent_log_root="logs/rlopt/ipmd/Isaac-Imitation-G1-Latent-v0"
        LATENT_LOW_LEVEL_CHECKPOINT="$(
            "${PYTHON_CMD[@]}" \
                experiments/interface_baselines/resolve_low_level_checkpoint.py \
                --log_root "${latent_log_root}" \
                --run_id "${LATENT_LOW_LEVEL_RUN_ID}" \
                --checkpoint_basename "${EXPECTED_LATENT_CHECKPOINT_BASENAME}"
        )"
        echo "[INFO] Resolved latent checkpoint from run ID: ${LATENT_LOW_LEVEL_CHECKPOINT}"
    fi
fi
...
if [[ ! -d "${DATASET_PATH}" && "${DRY_RUN}" != "1" ]]; then
    echo "[ERROR] Corrected LAFAN1 cache does not exist: ${DATASET_PATH}" >&2
    exit 2
fi
...
run_cmd "${ISAACLAB_PYTHON_CMD[@]}" \
    ...
    --label corrected_lafan1_direct_vanilla_50hz \
...
run_cmd "${ISAACLAB_PYTHON_CMD[@]}" \
    ...
    --label corrected_lafan1_streamed_vanilla_equivalence \
...
run_cmd "${ISAACLAB_PYTHON_CMD[@]}" scripts/rlopt/eval_skill_commander_closed_loop.py \
    ...
    --label corrected_lafan1_diffsr_latent_oracle \
    --num_envs "${NUM_ENVS}" \
...
run_cmd "${PYTHON_CMD[@]}" experiments/interface_baselines/write_interface_run_provenance.py \
    ...
    --note "strict 40-motion frame-0 oracle evaluation" \
    --note "Phase 4 remains blocked unless both audits and equivalence pass"

if [[ "${DRY_RUN}" == "1" || "${DRY_RUN}" == "true" ]]; then
    echo "[INFO] Corrected-LAFAN1 low-level qualification dry run rendered."
else
    echo "[PASS] Corrected-LAFAN1 low-level qualification passed: ${OUTPUT_ROOT}"
fi
