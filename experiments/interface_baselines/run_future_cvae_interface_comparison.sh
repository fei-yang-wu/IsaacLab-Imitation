...
# Two-stage causal planner protocol for the continuous Future-CVAE interface.
# Required inputs:
...
: "${LOW_LEVEL_CHECKPOINT:?Set LOW_LEVEL_CHECKPOINT to a trained Future-CVAE policy.}"
: "${MOTION_MANIFEST:?Set MOTION_MANIFEST to a one-motion manifest.}"

OUTPUT_ROOT="${OUTPUT_ROOT:-logs/interface_baselines/future_cvae}"
...
echo "[INFO] Future-CVAE comparison artifacts: ${OUTPUT_ROOT}"
