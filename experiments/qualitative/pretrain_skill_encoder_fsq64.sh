...
# Stage 1 of 2: pretrain the scaled SONIC-FSQ64 root_qpos DiffSR skill encoder
# on the full 129,785-motion BONES-SEED reference arrays.
#
...
# (`build_cluster_pretrain_command` + `append_pretrain_contract`, ICE job
# 5570673, encoder SHA-256 6a4a7248...da14). The only deltas are local
# execution: pixi instead of docker/cluster/cluster_interface.sh, a repo-local
# reference-arrays path, and the data gate below. Nothing that reaches the
# trainer changed.
#
# Terms used here:
#   DiffSR     - the successor-representation skill objective in
#                RLOpt/rlopt/agent/hl_skill_diffsr.py, pretrained offline and
#                then frozen for the tracker.
#   sonic_fsq  - finite scalar quantization publishing 64 coordinates at 32
#                levels each (~320 bits) directly as the command. This is NOT
#                --latent_mode fsq, which is a different head whose default
#                levels are (8,8,8,5,5) and which was near-flat in the
#                2026-07-22 ablation.
#   endpoint   - the transition objective: predict the window endpoint, not the
#                per-step occupancy.
#
# Self-contained by design: every constant is declared here, so an ablation is
# a single-file edit. Stage 2 is experiments/qualitative/train_lowlevel_fsq64.sh
# and declares its own copy. Anything the tracker also reads -- HORIZON_STEPS,
# Z_DIM, LATENT_MODE, the macro state terms, the dataset identity -- must change
# in both, or stage 2 refuses the encoder on its width assert.
#
...
# The tracker publishes z_dim + a 2-value sin_cos phase = 66. Printed here only
# so stage 1 and stage 2 can be read against each other.
...
# --sonic_fsq_levels is deliberately NOT passed: the campaign used the parser
# default, which is 64 coordinates x 32 levels, matching gear_sonic's (2, 32)
# tokens at num_fsq_levels=32. Passing it would be a contract change.
...
WANDB_TAGS="${WANDB_TAGS:-bones-seed,129785,v2,root-qpos,sonic-fsq64,scaled-pretrain,${TRANSITION_OBJECTIVE}}"
...
echo "[PLAN] contract    : 2026-08-06-bones129k-sonic-fsq-scale shared pretrain"
...
echo "[PLAN] latent      : ${LATENT_MODE} (64 coords x 32 levels), z${Z_DIM} -> ${LATENT_COMMAND_DIM} published by stage 2 with the sin_cos phase"
