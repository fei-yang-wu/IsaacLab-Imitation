...
# DETERMINISTIC-64 encoder from stage 1, on the full 129,785-motion BONES-SEED
# reference arrays.
#
#   DRY_RUN=1 bash experiments/qualitative/train_lowlevel_deter64.sh
#   DRY_RUN=1 TRACKER_ARM=sonic bash experiments/qualitative/train_lowlevel_deter64.sh
#   bash experiments/qualitative/train_lowlevel_deter64.sh
#
# This is the UNQUANTIZED arm of this directory's two-arm ablation. Every
# training argument is identical to train_lowlevel_fsq64.sh -- geometry, frame
# budget, gamma, reset schedule, termination curriculum, tracker capacity, and
# the 66-wide published command. The ONLY difference is which encoder it loads,
# which follows from LATENT_MODE below.
#
# The published command width is the same 66 (64 code values + a 2-value sin_cos
# phase) in both arms. That is deliberate: the actor sees an input of identical
# shape either way, so a difference in tracking is about what the 64 values MEAN,
...
# experiments/qualitative/pretrain_skill_encoder_deter64.sh and declares its
# own copy. HORIZON_STEPS, Z_DIM, LATENT_MODE, and the macro state terms must
# match the encoder being loaded -- the assert below catches a mismatch before
# training, so a pre-v2 670-wide encoder is refused rather than silently paired,
# and so is an FSQ encoder handed to this arm by accident.
...
# The published command is the 64 continuous latent values + the 2-value sin_cos
# phase -- the same width the FSQ arm publishes. Dropping the phase is
# catastrophic: episode length 21 against 144 on the 2026-08-02 screen.
...
# `deter64` here must match qualitative_env.sh's LATENT_ARM slug, or the
# qualitative launchers will not find this run by default.
LOWLEVEL_RUN_TAG="${LOWLEVEL_RUN_TAG:-bones129k_scaled_deter64_${TRACKER_ARM}_tracker_${BUDGET_LABEL}_seed${SEED}}"
TRAIN_LOG_DIR="${TRAIN_LOG_DIR:-${ABLATE_LOG_ROOT}/lowlevel/${LOWLEVEL_RUN_TAG}}"
WANDB_TAGS="${WANDB_TAGS:-bones-seed,129785,v2,root-qpos,deterministic64,lowlevel,${TRACKER_ARM}-tracker,newton,rollout${ROLLOUT_STEPS},gamma097,reset80-adaptive20}"
...
[[ -s "${ENCODER_CKPT}" ]] || fail "encoder checkpoint not found: ${ENCODER_CKPT}
  Run experiments/qualitative/pretrain_skill_encoder_deter64.sh first, or point
...
#
# The LATENT_MODE check is what keeps the two arms apart. An FSQ encoder has the
# same 380-wide input and the same 64-value z as a deterministic one, so the
# width assert alone would pair them happily and the run would silently be the
# other arm.
pixi run python -c '
import sys, torch
...
if mode != expected_mode:
    raise SystemExit(
        f"expected a {expected_mode} encoder, got {mode!r}. Both arms share this "
        "input width and z_dim, so this check is the only thing separating them."
    )
stray = [k for k in state_dict if "codebook" in k or "fsq" in k]
if stray:
    raise SystemExit(
        f"deterministic checkpoint carries quantizer weights {stray}; its config "
        "and its weights disagree"
    )
out_key, out_value = trunk[-1]
z_dim = int(checkpoint["config"]["z_dim"])
if int(out_value.shape[0]) != z_dim:
    raise SystemExit(
        f"deterministic trunk emits {int(out_value.shape[0])} values but z_dim is "
        f"{z_dim}: {out_key} {tuple(out_value.shape)}"
    )
print(
    f"[PASS] root_qpos {mode} encoder: {key} {tuple(value.shape)}, "
    f"continuous z_dim {z_dim} (no codebook, no lattice)"
)
' "${ENCODER_CKPT}" "${ENCODER_INPUT_WIDTH}" "${LATENT_MODE}"
...
echo "[PLAN] contract    : fsq64 low level against a deterministic encoder (unquantized arm)"
echo "[PLAN] tracker arm : ${TRACKER_ARM}"
...
echo "[PLAN] command     : ${Z_DIM} continuous + 2 phase = ${LATENT_COMMAND_DIM}, held ${LATENT_HOLD_STEPS} steps"
