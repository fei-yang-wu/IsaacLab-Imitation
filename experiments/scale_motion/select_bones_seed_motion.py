...
Examples:
    ...
    # Reproduce the pinned 5,000-motion subset. The two hash gates make a
    # changed source pool or selected list fail on every machine.
    pixi run python experiments/scale_motion/select_bones_seed_motion.py \
        ...
        --output-manifest data/bones_seed_sonic_129k_50hz/manifests/bones-seed-sonic-5000.json \
        --count 5000 --shuffle-seed 0 --require-files \
...
SONIC_FILTER_PATH = (
    REPO_ROOT / "scripts" / "data" / "filter_bones_seed_sonic_exclusions.py"
)


def _load_sonic_filter() -> tuple[list[str], Any]:
    spec = importlib.util.spec_from_file_location(
        "_bones_seed_sonic_exclusions", SONIC_FILTER_PATH
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load SONIC filter: {SONIC_FILTER_PATH}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return list(module.SONIC_RELEASE_FILTER_KEYWORDS), module.matched_keywords


SONIC_RELEASE_FILTER_KEYWORDS, matched_keywords = _load_sonic_filter()
...
def _require_sha256(*, label: str, actual: str, expected: str | None) -> None:
    if expected is None:
        return
    normalized = expected.strip().lower()
    if not re.fullmatch(r"[0-9a-f]{64}", normalized):
        raise ValueError(f"Invalid expected {label} SHA-256: {expected!r}")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Fail unless the source manifest has this SHA-256.",
    )
    parser.add_argument(
        "--expected-selected-names-sha256",
        default=None,
        help="Fail unless the ordered selected motion names have this SHA-256.",
    )
...
def main() -> None:
    ...
    _require_sha256(
        ...
        expected=args.expected_source_manifest_sha256,
    )
    ...
    _require_sha256(
        ...
        actual=selected_names_sha256,
        expected=args.expected_selected_names_sha256,
    )
    dataset_name = (
    ...
    metadata["sonic_exclusion_filter_source"] = (
        "scripts/data/filter_bones_seed_sonic_exclusions.py"
    )
    ...
    metadata["subset"] = {
        **method,
        ...
        "mirror_mode": args.mirror_mode,
        "selected_names_sha256": selected_names_sha256,
        **(
            {
